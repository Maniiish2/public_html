...
// 		exit;
	...
	public function module_status($id=''){
		...
		$result = $this->admin_roles->change_status_module($id,$status);
		if($result){
			$this->session->set_flashdata('success', 'Module status has been Updated to Active successfully!');
			redirect(base_url('admin/admin_roles/module'));
		}else{
			$this->session->set_flashdata('error','Module has been Deactively Successfully.');	
			redirect(base_url('admin/admin_roles/module'));
